...
pub fn run(av: &ArgValues) -> error::Result<()> {
    ...
    let window_config = glutin::WindowBuilder::new()
        .with_title("Shader Live Preview")
    ...
    let (window, mut device, mut factory, main_color, mut main_depth) =
        gfx_window_glutin::init::<ColorFormat, DepthFormat>(window_config, context, &events_loop).unwrap();

        window.set_position(LogicalPosition{x:920.0,y:0.0});
        ...
        events_loop.poll_events(|event| {
            ...
            if let Event::WindowEvent { event, .. } = event {
                match event {
                    ...
                    WindowEvent::Resized(LogicalSize{width:new_width, height:new_height}) => {
